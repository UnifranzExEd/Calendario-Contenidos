...
req = urllib.request.Request(f"{URL}/pestana_campos", headers=HEADERS)
with urllib.request.urlopen(req) as response:
    campos = json.loads(response.read())
    for c in campos:
        if c.get('pestana_id') == 1 and c.get('visible') == 1:
            print(f"Pestana 1 - {c.get('nombre_campo')}: {c.get('etiqueta')}")
